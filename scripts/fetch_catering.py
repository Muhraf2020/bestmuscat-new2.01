...
"""
Fetch Muscat catering services via Google Places (image-free, low-cost).

- Text Search + Details (NO 'photos' field) to keep API cost low
- Uses catering-related keywords (no dedicated Places 'caterer' type), radius-tiled over Muscat
- Writes to data/sources/catering.csv
- If catering.csv already exists, preserves its header order

Run (example):
  export GOOGLE_MAPS_API_KEY=YOUR_KEY
  python scripts/fetch_catering.py --basic-only --max-places 140
"""
...
def price_level_to_symbols(level):
    # Often absent for caterers; kept for compatibility
    mapping = {0:"$", 1:"$", 2:"$$", 3:"$$$", 4:"$$$$"}
    try:
...
DEFAULT_HEADERS = [
    ...
    # catering-specific placeholders (blank; enrich later)
    "wedding_catering","corporate_catering","event_catering","live_stations",
    "buffet_service","set_menu","veg_options","halal_certified",
    "delivery","setup_cleanup","tasting_available","min_headcount","max_headcount",
    "sample_menu_url","booking_url","quote_request_url"
]
...
def places_text_search(keyword: str, lat: float, lng: float, radius_m: int, page_token=None):
    """
    Text Search for catering services around a center point.
    There is no dedicated 'caterer' place type; we rely on keywords.
    (We do NOT set 'type' here to avoid filtering away real caterers.)
    """
...
def build_row(text_item: dict, details: dict, args, header_order):
    ...
    price_range = price_level_to_symbols(p.get("price_level"))  # often blank
    ...
    row_full = {
        ...
        # catering-specific placeholders (to be filled by enrichment)
        "wedding_catering": "",
        "corporate_catering": "",
        "event_catering": "",
        "live_stations": "",
...
def parse_cli():
    parser = argparse.ArgumentParser(
        description="Fetch Muscat catering services via Google Places (image-free, low-cost)."
    )
    parser.add_argument(
        "--keywords",
        type=str,
        # Use varied phrasing to capture actual caterers; we don't set a 'type'
        default="catering service,caterer,event catering,wedding catering,party catering,corporate catering,banquet catering,food catering,outdoor catering,live stations buffet",
    ...
    parser.add_argument("--radius", type=int, default=7000, help="Search radius per center in meters.")
    ...
    parser.add_argument("--details-throttle-sec", type=float, default=0.25,
...
def main():
    ...
    print(f"Fetching catering (image-free)… {len(keywords)} keywords × {len(centers)} centers "
    ...
    print(f"Wrote {len(rows)} unique catering businesses → {OUT_CSV}")
