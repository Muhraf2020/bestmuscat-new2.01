...
"""
Fetch Muscat shopping malls via Google Places (image-free, low-cost).

- Text Search + Details (NO 'photos' field) to keep API cost low
- Constrains results with type=shopping_mall
- Writes to data/sources/malls.csv
- If malls.csv already exists, preserves its header order

Run (example):
  export GOOGLE_MAPS_API_KEY=YOUR_KEY
  python scripts/fetch_malls.py --basic-only --max-places 80
"""
...
def price_level_to_symbols(level):
    # Uncommon for malls; kept for compatibility
    mapping = {0:"$", 1:"$", 2:"$$", 3:"$$$", 4:"$$$$"}
    try:
...
def favicon_url_for(site_url: str, size: int = 128) -> str:
    """Google S2 favicon (string only; no extra request here)."""
    try:
...
DEFAULT_HEADERS = [
    ...
    # mall-specific placeholders (blank; enrich later)
    "parking","cinema","kids_zone","food_court","supermarket",
    "hypermarket","fashion_focus","luxury_brands","indoor_theme_park",
    "opening_hours_json"
]
...
def places_text_search(keyword: str, lat: float, lng: float, radius_m: int, page_token=None):
    """
    Text Search for shopping malls around a center point.
    Constrains by 'type=shopping_mall' to reduce noise.
    """
...
def build_row(text_item: dict, details: dict, args, header_order):
    ...
    hours_json = ""
    if (not args.basic_only) and p.get("current_opening_hours"):
        try:
            hours_json = json.dumps(p["current_opening_hours"], ensure_ascii=False)
        except Exception:
            hours_json = ""
    ...
    row_full = {
        ...
        # mall-specific placeholders (optional enrichment later)
        "parking": "",
        "cinema": "",
        "kids_zone": "",
        "food_court": "",
...
def parse_cli():
    parser = argparse.ArgumentParser(
        description="Fetch Muscat shopping malls via Google Places (image-free, low-cost)."
    )
    parser.add_argument(
        "--keywords",
        type=str,
    ...
    parser.add_argument("--max-places", type=int, default=80, help="Stop after N unique places overall.")
    parser.add_argument("--wall-timeout-sec", type=int, default=900, help="Abort run after this many seconds.")
...
def main():
    ...
    print(f"Fetching malls (image-free)… {len(keywords)} keywords × {len(centers)} centers "
    ...
    print(f"Wrote {len(rows)} unique malls → {OUT_CSV}")
