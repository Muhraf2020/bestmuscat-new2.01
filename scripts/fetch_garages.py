...
"""
Fetch Muscat car repair garages via Google Places (image-free, low-cost).

- Text Search + Details (NO 'photos' field) to keep API cost low
- Constrains results with type=car_repair
- Writes to data/sources/garages.csv
- If garages.csv already exists, preserves its header order

Run (example):
  export GOOGLE_MAPS_API_KEY=YOUR_KEY
  python scripts/fetch_garages.py --basic-only --max-places 140
"""
...
def price_level_to_symbols(level):
    # Rare for garages; kept for compatibility
    mapping = {0:"$", 1:"$", 2:"$$", 3:"$$$", 4:"$$$$"}
    try:
...
DEFAULT_HEADERS = [
    ...
    # garage-specific placeholders (blank; enrich later if you like)
    "towing","roadside_assistance","oil_change","battery","tires",
    "wheel_alignment","ac_service","electrical","transmission",
    "body_shop","paint","detailing","wash","pickup_dropoff",
    "warranty","brand_specialist","dealership","open_24h","emergency_phone"
]
...
def places_text_search(keyword: str, lat: float, lng: float, radius_m: int, page_token=None):
    """
    Text Search for car repair around a center point.
    Constrains by 'type=car_repair' to reduce noise.
    """
...
def build_row(text_item: dict, details: dict, args, header_order):
    ...
    row_full = {
        ...
        "tags": "garage;car repair;mechanic",
        "neighborhood": "",
        "address": addr,
        "city": "Muscat",
        "country": "Oman",
...
def parse_cli():
    parser = argparse.ArgumentParser(
        description="Fetch Muscat car repair garages via Google Places (image-free, low-cost)."
    )
    parser.add_argument(
        "--keywords",
        type=str,
        default="car repair,auto repair,garage,mechanic,car workshop,service center,oil change,tires,battery,car detailing,car wash,body shop,paint,ac service,wheel alignment",
...
def main():
    ...
    print(f"Fetching garages (image-free)… {len(keywords)} keywords × {len(centers)} centers "
    ...
    print(f"Wrote {len(rows)} unique garages → {OUT_CSV}")
