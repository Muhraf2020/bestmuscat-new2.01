...
"""
Fetch Muscat home maintenance & repair businesses via Google Places (image-free, low-cost).

- Text Search + Details (NO 'photos' field) to keep API cost low
- Optionally constrains results with a supported Places 'type' when possible (per-keyword map)
- Writes to data/sources/home_maintenance.csv
- If the CSV already exists, preserves its header order; otherwise uses a default template

Run (example):
  export GOOGLE_MAPS_API_KEY=YOUR_KEY
  python scripts/fetch_home_maintenance.py --basic-only --max-places 160
"""
...
OUT_CSV = SRC_DIR / "home_maintenance.csv"
...
def price_level_to_symbols(level):
    # Not usually relevant; included for compatibility
    mapping = {0:"$", 1:"$", 2:"$$", 3:"$$$", 4:"$$$$"}
    try:
...
DEFAULT_HEADERS = [
    ...
    # home maintenance specific placeholders (blank; enrich later)
    "plumbing","electrical","ac_service","appliance_repair","painting",
    "carpentry","masonry","pest_control","cleaning","handyman",
    "water_tank_cleaning","solar_installation","waterproofing",
    "emergency_phone","open_24h","warranty","free_estimates","service_area",
    "booking_url","whatsapp_url"
]
...
# Some keywords map cleanly to official Places types; we use them to reduce noise when available.
KEYWORD_TYPE_HINT = {
    "plumber": "plumber",
    "plumbing": "plumber",
    ...
    "handyman": "point_of_interest",   # no official type; let keyword drive
    "ac service": "point_of_interest", # hvac_contractor not universally supported in Text Search
    "air conditioning": "point_of_interest",
    "appliance repair": "point_of_interest",
    "carpentry": "point_of_interest",
    "masonry": "point_of_interest",
    "pest control": "point_of_interest",
    "cleaning": "point_of_interest",
    "water tank cleaning": "point_of_interest",
    "waterproofing": "point_of_interest",
    "solar installation": "point_of_interest",
}

def places_text_search(keyword: str, lat: float, lng: float, radius_m: int, page_token=None):
    """
    Text Search around a center point.
    If a supported type hint exists for the keyword, include it to reduce noise.
    """
    url = "https://maps.googleapis.com/maps/api/place/textsearch/json"
    if page_token:
        params = {"pagetoken": page_token, "key": API_KEY}
    else:
        # Choose a type hint if available; fall back to point_of_interest for broad keywords
        key_lc = keyword.strip().lower()
        t = KEYWORD_TYPE_HINT.get(key_lc, "point_of_interest")
        params = {
            "query": keyword,
            "type": t,
            "location": f"{lat},{lng}",
...
def build_row(text_item: dict, details: dict, args, header_order):
    ...
    row_full = {
        ...
        "tags": "home repair;maintenance;services",
        "neighborhood": "",
        "address": addr,
        "city": "Muscat",
        "country": "Oman",
        ...
        # Home-maintenance placeholders (for a later enrich step)
        "plumbing": "",
        "electrical": "",
        "ac_service": "",
        "appliance_repair": "",
...
def parse_cli():
    parser = argparse.ArgumentParser(
        description="Fetch Muscat home maintenance & repair via Google Places (image-free, low-cost)."
    )
    parser.add_argument(
        "--keywords",
        type=str,
        # Feel free to adjust. Type hints above reduce noise for some of these.
        default="plumber,electrician,painter,locksmith,roofing contractor,general contractor,handyman,ac service,air conditioning,appliance repair,carpentry,masonry,pest control,cleaning,water tank cleaning,waterproofing,solar installation",
    ...
    parser.add_argument("--max-places", type=int, default=160, help="Stop after N unique places overall.")
...
def main():
    ...
    print(f"Fetching home maintenance (image-free)… {len(keywords)} keywords × {len(centers)} centers "
    ...
    print(f"Wrote {len(rows)} unique businesses → {OUT_CSV}")
