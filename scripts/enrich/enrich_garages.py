...
# -*- coding: utf-8 -*-
"""
Enrich garages CSV with free web signals (no paid APIs).
...
- Detects useful links: services/pricelist, booking/appointments, WhatsApp/call link
- Heuristically flags common garage services (towing, tires, AC, etc.)
- Attempts to extract an emergency phone number
- Aggregates a simple semicolon-separated `amenities` string
- DOES NOT download images; only lightweight HEAD/GET checks

Run:
  python scripts/enrich/enrich_garages.py --csv data/sources/garages.csv
"""
...
def first_garage_like(ldjson_list: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    # Common types used by garages
    pref = {"autorepair", "automotivebusiness", "localbusiness"}
...
# ---------- Garage-specific detectors ----------
SERVICES_SETS = {
    "towing": ("towing", "recovery service", "flatbed"),
    "roadside_assistance": ("roadside assistance", "breakdown service"),
    "oil_change": ("oil change", "engine oil", "lube service"),
    "battery": ("battery", "jump start", "battery replacement"),
    "tires": ("tyre", "tire", "wheel", "puncture", "balancing"),
    "wheel_alignment": ("wheel alignment", "alignment", "camber"),
    "ac_service": ("a/c service", "ac service", "aircon", "air conditioning"),
    "electrical": ("electrical", "auto electrician", "diagnostics"),
    "transmission": ("transmission", "gearbox", "clutch"),
    "body_shop": ("body shop", "bodywork", "panel beating"),
    "paint": ("paint", "painting", "spray booth", "respray"),
    "detailing": ("detailing", "polishing", "ceramic coating"),
    "wash": ("car wash", "wash", "valet"),
    "pickup_dropoff": ("pickup", "pick-up", "collection", "dropoff", "drop off"),
    "warranty": ("warranty", "guarantee"),
    "brand_specialist": ("specialist", "expert", "authorized"),
    "dealership": ("dealer", "dealership", "authorized dealer"),
    "open_24h": ("24/7", "24x7", "open 24", "twenty four hours"),
}

BOOKING_WORDS = ("book", "booking", "appointment", "reserve", "schedule")
SERVICE_PAGE_WORDS = ("services", "our services", "what we do", "price list", "rates", "pricelist")
...
def detect_emergency_phone(text: str) -> str:
    # crude extraction; prefer the first phone-like number appearing near "emergency"
    t = text.lower()
    idx = t.find("emergency")
    if idx >= 0:
        window = text[max(0, idx - 80): idx + 120]
        m = PHONE_RE.search(window)
        if m:
            return m.group(0)
    # fallback: first phone-looking string on the page
...
def enrich_row(sess: requests.Session, row: Dict[str, str]) -> Dict[str, str]:
    ...
    garage_ld = first_garage_like(ld) if ld else None
    ...
    # JSON-LD enrichments: sometimes priceRange / amenities exist
    if garage_ld:
        if not clean(row.get("price_range")) and garage_ld.get("priceRange"):
            row["price_range"] = clean(garage_ld.get("priceRange"))
        if not clean(row.get("amenities")):
            amen = []
            af = garage_ld.get("amenityFeature")
    ...
    # Links + services from page text
    if soup and page_resp is not None:
        base = page_resp.url
        txt = soup.get_text(" ", strip=True)

        # Useful links
        if not clean(row.get("services_url")):
            su = find_first_link_containing(soup, base, SERVICE_PAGE_WORDS)
            if is_http(su): row["services_url"] = su
        ...
        for field, words in SERVICES_SETS.items():
            set_yes(field, words)

        # Emergency phone (best-effort)
        if not clean(row.get("emergency_phone")):
            ep = detect_emergency_phone(txt)
            if ep: row["emergency_phone"] = ep
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description="Enrich garages CSV (free sources only).")
    ap.add_argument("--csv", default="data/sources/garages.csv",
                    help="Path to garages CSV (will be updated in place).")
    ...
    with path.open(newline="", encoding="utf-8") as f:
        ...
        must_have = [
            ...
            "services_url","booking_url","whatsapp_url",
            "towing","roadside_assistance","oil_change","battery","tires","wheel_alignment",
            "ac_service","electrical","transmission","body_shop","paint","detailing","wash",
            "pickup_dropoff","warranty","brand_specialist","dealership","open_24h","emergency_phone",
            "amenities"
        ]
