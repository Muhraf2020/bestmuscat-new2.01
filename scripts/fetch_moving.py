...
"""
Fetch Muscat moving & storage providers via Google Places (image-free, low-cost).

- Text Search + Details (NO 'photos' field) to keep API cost low
- Constrains results with types: moving_company + storage (via keywords + type)
- Writes to data/sources/moving.csv
- If moving.csv already exists, preserves its header order

Run (example):
  export GOOGLE_MAPS_API_KEY=YOUR_KEY
  python scripts/fetch_moving.py --basic-only --max-places 140
"""
...
def price_level_to_symbols(level):
    # Rare/undefined for this vertical; kept for compatibility
    mapping = {0:"$", 1:"$", 2:"$$", 3:"$$$", 4:"$$$$"}
    try:
...
DEFAULT_HEADERS = [
    ...
    # moving/storage-specific placeholders (blank; enrich later)
    "local_moves","international_moves","office_moves","packing","unpacking",
    "storage","climate_control","boxes_supplies","insurance","piano_moves",
    "vehicle_transport","disposal","24_7","emergency_phone","quote_url","booking_url"
]
...
def places_text_search(keyword: str, lat: float, lng: float, radius_m: int, page_token=None):
    ...
    We keep it broad on query keywords but constrain with type 'moving_company' for most lookups.
    Some keywords (e.g., storage) still benefit from the same type due to Google matching; we rely on keywords to broaden.
    """
...
def build_row(text_item: dict, details: dict, args, header_order):
    ...
    row_full = {
        ...
        "tags": "moving;relocation;storage;packers",
        "neighborhood": "",
        "address": addr,
        "city": "Muscat",
        "country": "Oman",
...
def parse_cli():
    parser = argparse.ArgumentParser(
        description="Fetch Muscat moving & storage providers via Google Places (image-free, low-cost)."
    )
    parser.add_argument(
        "--keywords",
        type=str,
        default="moving company,movers,relocation,house shifting,office relocation,packing service,storage,self storage,warehouse,box supplies,removal company,international movers,local movers",
...
def main():
    ...
    print(f"Fetching moving/storage (image-free)… {len(keywords)} keywords × {len(centers)} centers "
    ...
    print(f"Wrote {len(rows)} unique moving/storage providers → {OUT_CSV}")
