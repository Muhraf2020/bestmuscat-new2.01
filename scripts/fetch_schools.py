...
"""
Fetch Muscat schools via Google Places (image-free, low-cost).

- Uses Places Text Search + Details (NO photos) to keep costs low.
- Writes to data/sources/schools.csv.
- If data/sources/schools.csv already exists, its **header order** is preserved.
- Otherwise, we fall back to a sensible default schools template.

Run (example):
  export GOOGLE_MAPS_API_KEY=YOUR_KEY
  python scripts/fetch_schools.py --basic-only --max-places 120
"""
...
def price_level_to_symbols(level):
    # Schools generally don't expose this; keep for compatibility (will be blank)
    mapping = {0:"$", 1:"$", 2:"$$", 3:"$$$", 4:"$$$$"}
    try:
...
def places_text_search(keyword: str, lat: float, lng: float, radius_m: int, page_token=None):
    """
    Text Search for schools around a center point.
    Constrains by 'type=school' to reduce noise.
    """
...
DEFAULT_HEADERS = [
    ...
    # school-specific placeholders (kept blank; optional enrichment later)
    "curriculum","grades_offered","language_of_instruction","coed","boarding",
    "transport","admissions_url","apply_url","tuition_currency","tuition_min","tuition_max"
]
...
def normalize_row(d, header_order):
    # keep only columns the header expects; fill missing
    out = {}
...
def build_row(text_item: dict, details: dict, args, header_order):
    ...
    price_range = price_level_to_symbols(p.get("price_level"))  # likely blank

    maps_url = maps_place_url(p.get("place_id",""))
    if not web:
        web = maps_url

    # (free) favicon string (no request performed here)
    logo_online = ""
    ...
    # Compose a full row (superset), then normalize to header_order
    row_full = {
        "id": p.get("place_id",""),
        "slug": slug,
        "name": name,
        ...
        # school-specific placeholders (blank for enrichment)
        "curriculum": "",
        "grades_offered": "",
...
def parse_cli():
    parser = argparse.ArgumentParser(
        description="Fetch Muscat schools via Google Places with low-cost, image-free details."
    )
    parser.add_argument(
        "--keywords",
        type=str,
        # You can expand/trim; 'type=school' already constrains results
        default="school,primary school,secondary school,high school,international school,kindergarten,nursery,college,academy",
    ...
    parser.add_argument("--max-places", type=int, default=150, help="Stop after N unique places overall.")
...
def main():
    ...
    # Decide the header order (existing CSV first, else default template)
    ...
    print(f"Fetching schools (image-free)… {len(keywords)} keywords × {len(centers)} centers "
    ...
    print(f"Wrote {len(rows)} unique schools → {OUT_CSV}")
