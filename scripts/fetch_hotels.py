...
# Robust GET with simple retries/backoff
...
def places_text_search(keyword: str, lat: float, lng: float, radius_m: int, page_token=None):
    ...
    if page_token:
        ...
        params = {
            "query": keyword,
            "type": "lodging",      # bias results toward hotels/lodging
            "location": f"{lat},{lng}",
            "radius": radius_m,
            "key": API_KEY,
        }
...
def build_row(text_item: dict, details: dict, args):
    ...
    hero_local = ""
    if (not args.no_photos) and chosen_ref:
...
def fetch_for_center_keyword(lat, lng, radius_m, keyword, rows_by_pid, args):
    ...
    while True:
        ...
        # Duplicate page guard (Google re-served same set)
