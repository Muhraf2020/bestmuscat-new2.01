...
"""
Fetch Muscat spas via Google Places (image-free, low-cost).

- Uses Places Text Search + Details (NO photos) to keep costs low.
- Writes to data/sources/spas.csv.
- If data/sources/spas.csv already exists, its **header order** is preserved.
- Otherwise, uses a sensible default spa template (see DEFAULT_HEADERS).

Run (examples):
  export GOOGLE_MAPS_API_KEY=YOUR_KEY
  python scripts/fetch_spas.py --basic-only --max-places 120
  python scripts/fetch_spas.py --keywords "spa,hamam,wellness center,beauty spa"
"""
...
def places_text_search(keyword: str, lat: float, lng: float, radius_m: int, page_token=None):
    ...
    Constrain by 'type=spa' to reduce noise.
    """
...
# ─────────────────────── CSV schema ───────────────────────
# If spas.csv already exists, we’ll keep its header order. Otherwise use this default.
DEFAULT_HEADERS = [
    ...
    # spa-specific placeholders (blank for enrichment later)
    "services_offered","sauna","steam_room","hammam","jacuzzi","pool",
    "massage_types","ladies_only_hours","couples_rooms","family_friendly","reservations_url"
]
...
def build_row(text_item: dict, details: dict, args, header_order):
    ...
    price_range = ""  # spas usually don't expose a price_level; keep blank

    maps_url = maps_place_url(p.get("place_id",""))
    if not web:
        web = maps_url

    # (free) favicon string (no extra request to your quota)
    logo_online = ""
    ...
    row_full = {
        ...
        "hero_url": "",                 # keep blank (enrich later)
        "image_credit": "",
        "image_source_url": "",
        ...
        # spa-specific placeholders (filled later by an enrich_spas.py if you add one)
        "services_offered": "",
        "sauna": "",
        "steam_room": "",
        "hammam": "",
...
def parse_cli():
    parser = argparse.ArgumentParser(
        description="Fetch Muscat spas via Google Places with low-cost, image-free details."
    )
    parser.add_argument(
        "--keywords",
        type=str,
        # You can expand/trim as you like; 'type=spa' already constrains results
        default="spa,wellness center,beauty spa,hamam,hammam,steam,sauna,thai massage,balinese massage",
    ...
    parser.add_argument("--max-places", type=int, default=120, help="Stop after N unique places overall.")
...
def main():
    ...
    # Decide header order (existing CSV first, else default template)
    ...
    print(f"Fetching spas (image-free)… {len(keywords)} keywords × {len(centers)} centers "
    ...
    print(f"Wrote {len(rows)} unique spas → {OUT_CSV}")
    print("Note: hero_url intentionally left blank. Use enrichment steps later to add images.")
